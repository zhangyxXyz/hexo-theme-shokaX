...
The blog's `toolbox/resource-localization` now provides `node toolbox/resource-localization/index.mjs snapshot --only waline --refresh --apply` to download this release and verify its pinned archive and client SHA-256 before updating the snapshot. The source is a GitHub Release, not the official npm client. Normal builds still use the reviewed local file. Release metadata is stored in `version.json`; a replaced release with the same tag is rejected until its hashes are explicitly reviewed and updated.
